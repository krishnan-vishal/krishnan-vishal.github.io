...
assert.match(tickerReaderSource, /updated\.textContent = `\$\{statusText\} · Updated \$\{timeText\}`/, "ticker status and time must render as one compact line");
...
assert.deepStrictEqual(fxConfig.regionCurrencyMap["SOUTH ASIA"], ["INR", "BDT", "NPR", "LKR", "MVR", "BTN", "AFN"], "South Asia must map destination currencies, not INR-linked pairs");
assert.deepStrictEqual(fxConfig.regionCurrencyMap["GCC / MIDDLE EAST"].slice(0, 6), ["AED", "SAR", "QAR", "KWD", "BHD", "OMR"], "GCC destination currencies must remain correctly grouped");
assert.ok(fxConfig.regionCurrencyMap.EUROPE.includes("GBP"), "GBP must be available as a European destination currency");
...
Object.values(fxConfig.regionCurrencyMap).flat().forEach(code => assert.ok(providerCurrencies.has(code), `${code} must exist in the validated provider universe`));
