...
const fxConfig = JSON.parse(fs.readFileSync(path.join(__dirname, "..", "assets", "data", "fx", "fx-config.json"), "utf8"));
...
assert.ok(fxConfig.treasurySendingBases.includes("GBP"), "GBP must also remain a Treasury sending currency");
const providerCurrencies = new Set(currentSnapshot.currencyUniverse.currencies);
assert.match(fxAppSource, /findOrDeriveRecord\(snapshot, `\$\{base\}\/\$\{destination\}`\)/, "Treasury cards must be generated by sending base into selected destination");
assert.doesNotMatch(fxAppSource, /GCC_MANAGED_CURRENCIES|classifyPair/, "Treasury must not use legacy pair-membership grouping");
assert.ok(countryRegistry.countries.length >= 185, "the FX registry must cover GPIR's global destination-country set");
countryRegistry.countries.forEach(country => {
    assert.match(country.countryCode, /^[A-Z]{2}$/, `${country.country} must have an ISO country code`);
    assert.match(country.currencyCode, /^[A-Z]{3}$/, `${country.country} must have an ISO currency code`);
    assert.strictEqual(typeof country.enabled, "boolean", `${country.country} must declare enabled state`);
});
const enabledCurrencies = countryRegistry.countries.filter(country => country.enabled).map(country => country.currencyCode);
assert.ok(new Set(enabledCurrencies).size >= 130 && new Set(enabledCurrencies).size < enabledCurrencies.length, "supported currencies must provide broad coverage while shared currencies deduplicate to one destination-rate identity");
assert.ok(countryRegistry.countries.filter(country => country.currencyCode === "EUR").length > 10, "separate EUR countries must remain represented");
const currenciesForRegion = region => new Set(countryRegistry.countries.filter(country => country.enabled && country.region === region).map(country => country.currencyCode));
assert.ok(["INR", "BDT", "NPR", "LKR", "MVR", "BTN", "AFN", "PKR"].every(code => currenciesForRegion("SOUTH ASIA").has(code)), "South Asia must map its supported destination currencies");
assert.ok(["AED", "SAR", "QAR", "KWD", "BHD", "OMR"].every(code => currenciesForRegion("GCC / MIDDLE EAST").has(code)), "GCC destination grouping must remain correct");
assert.ok(currenciesForRegion("EUROPE").has("GBP"), "GBP must remain a European destination currency");
