...
if(!tickerSource.includes('class="ticker-sequence"') || !tickerSource.includes("cloneNode(true)")) fail("continuous two-sequence ticker contract is missing");
if(tickerSource.includes("sequenceHTML + sequenceHTML")) fail("ticker must clone one canonical sequence rather than concatenate duplicate record output");
...
if(!archivePage.includes("data-live") || !archivePage.includes("data-period-nav") || !archivePage.includes("data-archive")) fail("archive live/month-year/history sections are incomplete");
if(/data-(?:live|archive)><\/div>/.test(archivePage)) fail("archive must include a server-rendered fallback rather than blank data containers");
...
if(!archivePage.includes("data-canonical-announcements") || archiveRuntime.includes("fetch(")) fail("archive filters must use the embedded canonical snapshot without a runtime fetch");
if(archivePage.includes('data-filter="from"') || archivePage.includes('data-filter="to"')) fail("date-range controls must not remain the primary archive workflow");
