...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| STAGING-TICKER-01 | 2026-09-15 | Build and verify a compact Supabase announcement ticker on an isolated staging branch | READER EXPERIENCE, DATA, GOVERNANCE | High | IN PROGRESS | Publicly readable approved/ticker-eligible Supabase rows; owner staging review | Stage 1 | `index.html`, `assets/js/supabase-announcements.js`, `assets/css/supabase-ticker.css`, `scripts/test-supabase-ticker.js` | Read the latest 20 eligible rows, render a dense continuous ticker and accessible narration/source dialog, retain canonical last-known-good content, and leave `main` and core layout styles unchanged. | Branch-isolated implementation; query/safety/fallback tests; 32px moving browser result; dialog keyboard flow; repository integrity gates pass; live eligible rows visible under the public key before production review. | Implementation and local browser/integrity validation complete; branch pushed and remotely verified. Public endpoint returns zero readable rows, so canonical fallback was verified and live-row acceptance remains pending. | PENDING owner staging review | `14a3612` | STAGING-TICKER-01 |
