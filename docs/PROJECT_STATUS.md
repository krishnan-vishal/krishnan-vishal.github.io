...
- **Milestone:** Global Announcements — permanent production recovery and source-to-intelligence pipeline.
- **Category / owner:** Intelligence / Reader Experience / Automation / Governance; Vishal Krishnan.
- **Objective:** Repair the source-to-publication break, prevent blank reader states, generate useful source-derived intelligence, retain permanent history and expose honest operational health without adding sources or a runtime AI dependency.
- **Working branch:** `work/global-announcements-production-recovery` from `origin/main` at `0438eae` (merged PR #338). Suggested PR title: `Global Announcements: permanent source-to-intelligence production pipeline`.
- **Implementation status:** IMPLEMENTED / LOCALLY VALIDATED / LOCAL BROWSER QA PASSED / PENDING COMMIT, PUSH, PR, ACTIONS AND OWNER REVIEW. No merge is authorised.
- **Root cause fixed:** discovery previously ended in the non-public candidate queue; no deterministic candidate-to-canonical-record promotion/generation stage existed. The archive's primary sections were empty HTML containers populated only by JavaScript, and freshness copy was derived from static/legacy dates.
- **Publication contract:** active `VERIFIED_OFFICIAL` Tier-1 primary sources may publish only after current-cycle source health, official HTTPS domain, publication date, strict payment relevance, completeness and URL/event deduplication all pass. Lower-tier, ambiguous, unhealthy and duplicate records remain non-public. Automation writes only `automation/intelligence-candidates`; `main` and publication remain human-merge controlled.
- **Reader contract:** generated HTML includes non-blank server-rendered Live, Latest and Historical sections; client loading enhances rather than creates the reader. Date-only items never receive fabricated time and go directly to archive. Search and ASK GPIR continue to read `announcements.json`; all published records have permanent detail URLs and original-source links.
- **Controlled backfill (2026-08-15 through 2026-09-09):** 88 registered / 13 active sources; retry pass discovered 112 in-window records, added one HKMA candidate, rejected 106 as non-relevant and suppressed five duplicate URLs. The earlier pass contributed five retained review candidates. Exactly one qualified official record was promoted: HKMA's 4 September contactless mobile-payment phishing notice. Five candidates remain non-public.
- **Current lifecycle:** 0 live, 10 archived published records, 1 developing announcement and 5 proposal candidates awaiting review. Source snapshot: 88 registered, 13 active/healthy, 75 unsupported, 0 degraded/failed/stale on the successful retry cycle.
- **Files / controls:** candidate publisher, bounded retrieval retry/backoff, health snapshot, generator/dashboard/ticker, announcement validator/regressions, existing two-hour workflow, canonical datasets/pages/sitemap and governance handoff. The ticker now renders one compact scrollable sequence with no duplicate records.
- **Validation:** content, internal links (100 HTML files), announcements, lifecycle, production pipeline (28 checks), Search/ASK intent, radar/parser isolation, M24 reader integration, source activation, edited-script syntax, diff whitespace and local browser rendering/navigation passed. Performance audit retains four pre-existing advisory warnings and adds none. Remaining: commit/push, confirm remote SHA, create one PR, await Actions and owner approval. Production remains unchanged until merge.
