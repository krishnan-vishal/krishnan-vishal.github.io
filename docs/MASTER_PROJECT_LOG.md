...
## M33-G1 Step 7F.1 — claim RPC runtime hotfix

- **Production evidence and root cause:** the owner's first deterministic SFA claim attempt failed closed with PostgreSQL `42883` before any lease or Edge invocation because the deployed function called `pg_catalog.greatest` and `pg_catalog.least`. PostgreSQL implements `GREATEST`/`LEAST` as conditional expressions rather than schema-resolved ordinary functions; the correct hardened syntax is unqualified. Scheduler remains OFF, the legacy writer remains disabled and the owner-verified baseline is RAW=9, candidates=9, rejections=0, handoffs=0 and announcements=77.
- **Minimal repair:** `m33-g1-source-run-claim.sql` changes only the invalid qualifications. The owner hotfix uses `CREATE OR REPLACE FUNCTION` in a transaction, retains the exact signature/default, 60–1800 bound, advisory/row locking, finite lease, overlap counters, matching-token release contract, `SECURITY DEFINER`, `search_path=pg_catalog`, qualified application objects and service-role-only EXECUTE. It does not drop or change the claim table, alter RLS, touch intelligence data, deploy Edge code or activate Cron.
- **Regression correction:** the prior static test encoded the invalid spelling as an expected pattern, and the isolated PostgreSQL workflow ran the Step 6 package but never applied/executed the Step 7 claim migration. Static coverage now rejects either invalid qualification. The disposable PostgreSQL job applies the claim migration and idempotent hotfix, then executes default/minimum/maximum TTL, same-source overlap, wrong-token/matching-token release, RLS, search-path and grant behavior in a rollback-only runtime test.
- **Owner next step:** execute only `migrations/m33-g1/m33-g1-step-7f1-claim-rpc-hotfix.sql`, then run the SELECT-only `m33-g1-step-7f1-claim-rpc-hotfix-verification.sql`. Resume the overlap proof only after every verification field passes. Scheduler activation is not prepared or authorized by this hotfix.

## M33-G1 Step 7C — least-privilege orchestration safety-layer package

- **Objective and security decision:** finalize the Step 7A package after owner cloud preflight without deploying it. The existing Edge Function uses `withSupabase({ auth: ["secret"] })` and invokes RPC through `ctx.supabaseAdmin`; the framework and Supabase key contracts map that administrative client to the RLS-bypassing `service_role`. The migration therefore revokes new table/function access from `PUBLIC`, `anon`, `authenticated` and any prior `service_role` grant, withholds direct table access, and grants only the two fixed claim RPCs to `service_role`. `SECURITY DEFINER` is retained to keep the runtime surface narrower than table DML; `search_path=pg_catalog`, qualified objects, RLS and matching-token/finite-lease controls remain. Existing GPIR function grants are untouched.
- **Deployment package:** the scheduler now contains the verified existing endpoint, independent SFA/RBI/PYMNTS jobs at 05/25/45 UTC and runtime-only Vault lookup. Missing/null/empty `gpir_edge_function_secret` produces no HTTP call; no credential is hardcoded. Emergency disable remains limited to those three jobs. New SELECT-only post-migration verification proves table/RLS presence, function owner/signatures/definer/search-path state, role grants, no M33 Cron and unchanged RAW=9/candidates=9/rejections=0/handoffs=0/announcements=39 baseline without revealing Vault data. Dry runs remain claim-free/write-free; production write mode claims before fetch.
- **Validation:** Step 6A static migration (414 checks), 35/35 fixture contract, Step 6E B2/D2/F2/F5/F14/G1/G6, PYMNTS onboarding, boundary, data-contract and Step 7C safety tests pass. Content, link, announcement, announcement-intent, repository JavaScript syntax, high-confidence secret scan and `git diff --check` also pass. Behavioral PostgreSQL/Deno/live tests were not executed locally; this is recorded, not treated as a new architecture blocker.
- **Boundary / deployment order:** no Supabase connection, SQL execution, Edge deployment/invocation, Cron creation, candidate processing, handoff, publication, announcement, ticker, PR, main merge or legacy-grant change occurred. Owner sequence is claim migration → read-only verification → Edge deployment → normal invocation → same-source overlap proof → safe different-source proof → boundary/count verification. Scheduler is eligible only for a separate later owner activation after every check passes. Delivery subject: `M33-G1 Step 7C finalize least privilege orchestration safety` on `work/m33-g1-global-intelligence-engine`.

## M33-G1 Step 7B — owner cloud preflight

- **Production evidence:** PASS. The existing endpoint is `https://qlnvhfapctcpzqyuhhth.supabase.co/functions/v1/gpir-intelligence-fetch`; installed versions are `pg_cron` 1.6.4, `pg_net` 0.20.4 and `supabase_vault` 0.3.1. Vault contains `gpir_edge_function_secret`; its value was not exposed. No M33/GPIR intelligence Cron job, source/run claim function, or source/run/ingestion claim table existed.
- **Permission evidence:** several existing GPIR functions have broad EXECUTE grants, and `gpir_process_raw_record` / `gpir_process_raw_batch` include `anon` and `authenticated`. Step 7B records this evidence only; Step 7C must not change those already-proven pipeline grants and limits least privilege to the new claim functions.
- **Baseline / boundary:** RAW=9, candidates=9, rejections=0, handoffs=0 and global announcements=39; Cron absent, ticker unchanged, publication closed and handoff closed. Preflight did not itself authorize claim migration, Edge deployment, production invocation or scheduler activation.

## M33-G1 Step 7A — orchestration safety layer and scheduler preparation

- **Safety mechanism:** the existing Edge Function now makes a write-mode claim before creating `intelligence_ingestion_runs` or fetching a source. A finite persistent per-source claim is atomic through a short transaction-scoped advisory lock and row lock. Session advisory locking alone cannot safely span separate pooled Edge RPC/database requests and external fetches; the finite lease is the minimal reliable lifecycle guard. Same-source overlap returns `SKIPPED_OVERLAP`/`SOURCE_RUN_ALREADY_ACTIVE` with zero fetch/RAW/processor work; different sources remain independent. A matching-token `finally` release and 60–1800 second expiry (900 requested) prevent starvation after errors/crashes. Overlap count/timestamp remain observable without fabricating a successful ingestion run.
- **Scheduler artifacts:** owner-only `pg_cron`/`pg_net` preparation targets exactly SFA at 05, RBI at 25 and PYMNTS at 45 UTC using the existing Edge Function with `dry_run=false` only in the unexecuted artifact. The existing Vault secret `gpir_edge_function_secret` is read at job runtime and is never hardcoded. The activation SQL fails closed until an owner binds the exact existing endpoint absent from repository evidence; a separate emergency-disable artifact touches only the three named M33 jobs.
- **Boundary and status:** RLS remains enabled; no guessed runtime role grant is made. RAW-first, individual processing, MAX_PAGE_FETCHES=3, dedupe, handoff/publication/ticker/Cron boundaries and SFA/RBI/PYMNTS-only authorization remain intact; Fintech Futures and unknown sources stay blocked. Static safety, boundary, data-contract and Step 6A validators pass. No Supabase connection, migration execution, deployment, scheduler activation, invocation, RAW/candidate/handoff/announcement/ticker change, PR or main action occurred.
...
- **Blocker:** requested B2 run-accounting columns conflict with the verified repository baseline and are not evidenced in deployed Supabase metadata. The instruction requires no guessed mapping.
- **Impact:** no function edit, Supabase contact, deployment, canary, publication, Cron, ticker, workflow or main change occurred.
...
- **Classification / objective:** ARCHITECTURE, DATA, GOVERNANCE and SECURITY planning milestone. Freeze the implementation-ready contract for the smallest safe Gate 2 integration, three-way routing, versioned taxonomy, durable lineage, validation-to-canonical-handoff boundary, controlled rollout and rollback without implementing or applying it.
- **Architecture decision:** retain the six verified Supabase tables and four functions. Reuse `intelligence_candidates.candidate_status='REVIEW'` as quarantine and `PENDING` as validation-required Gate-2 candidate. Add only missing run/assessment/taxonomy lineage and one idempotent handoff record; keep `global_announcements` publication-only and the existing M30 layer authoritative.
- **Processor contract:** Gate 1 rejects retain RAW and namespaced reasons; every survivor calls Gate 2 exactly once; Gate-2 REJECT records a terminal rejection; REVIEW remains ticker-ineligible and cannot hand off; CANDIDATE remains PENDING/ticker-ineligible until deterministic validation. Single and batch processors have no publication/handoff write privilege.
- **Taxonomy / lineage / handoff:** freeze 22 canonical family IDs at taxonomy version `M33-G1-TAXONOMY-1`, preserve UNKNOWN and multi-topic evidence, trace source→run→RAW→assessment→outcome→validation→handoff, protect RAW links with future RESTRICT/least-privilege controls, and use a unique deterministic handoff key. Existing IDs/records are never replaced or silently reclassified.
- **Publication/security contract:** the future minimal default correction is `global_announcements.ticker_eligible DEFAULT false` with no update to existing rows. SECURITY DEFINER processors require fixed search paths, qualified objects, verified least-privilege callable roles, no dynamic SQL, fail-closed transactions and no mutation privilege on publication or handoff.
- **Regression/canary/rollback:** the non-runtime fixture defines 35 cases covering all required Gate 1/Gate 2, taxonomy, dedupe, timestamp, batch, lineage, handoff, LKG and legacy controls. The future canary uses one owner-selected verified source from each of three classes and advances through dry-run, RAW-only, gate, validation, handoff, publication and lifecycle proof with explicit STOP criteria. Rollback restores captured processor definitions/grants while retaining all evidence/history and the legacy production path.
- **Files created:** `docs/M33-G1-IMPLEMENTATION-REGRESSION-SPEC.md`; `tests/fixtures/m33-g1-regression-cases.json`.
- **Control records updated:** `docs/PROJECT_STATUS.md`; `docs/GPIR_BACKLOG.md`; `docs/MASTER_PROJECT_LOG.md`, including exact Step 5E commit evidence and achieved Step 5F specification state.
- **Files deliberately untouched:** Supabase/schema/functions/data; `scraper.js`; workflows; canonical data; pages; homepage; ticker UI/CSS/JS; CNAME; existing M30 runtime/publication implementation.
- **Validation:** fixture JSON and 35-case required-field coverage pass. Step 5B boundary passes with the known legacy baseline, 0 new violations and 0 protected changes. Step 5C contract passes with 8/8 entities and 22/22 taxonomy families. Content, announcement, announcement-intent, 29-check production pipeline, 104-file link and whitespace validations pass.
- **Milestone outcome / production impact:** STEP 5F PLANNING/SPECIFICATION MILESTONE ACHIEVED on the work branch. No executable migration or runtime implementation was created; no main, Supabase, `global_announcements`, ticker, workflow, deployment, source activation or live GPIR change occurred.
- **Delivery / next dependency:** commit and push only to the named work branch under subject `M33-G1 Step 5F specify Gate2 rollout`; exact SHA is available in Git history and verified in the final handoff. Next is owner review and one separately authorized Step 6A milestone to build version-controlled SQL migration and isolated regression artifacts only, not apply them.
...
- **Classification / objective:** ARCHITECTURE, DATA, GOVERNANCE and SECURITY inspection/reconciliation milestone. Reconcile authoritative metadata manually collected by the owner through authenticated Supabase SQL Editor read-only queries against the Step 5C logical contract. This workspace did not connect to Supabase or independently execute the queries.
- **Non-mutation evidence:** no production business rows were read; no function was executed; no SQL, DDL, data mutation, credential use/exposure, deployment, workflow, scheduler, publication or ticker change occurred.
- **Physical evidence:** six tables and their supplied columns/keys/constraints/indexes are recorded: `source_registry`, `intelligence_raw_ingestion`, `intelligence_candidates`, `intelligence_rejection_log`, `intelligence_ingestion_runs`, `global_announcements`. RLS is enabled on all; no explicit policies were returned by `pg_policies`; trigger metadata is unverified. Four signatures/properties are verified: immutable/invoker assessment and rejection functions plus volatile/security-definer single/batch RAW processors. Function bodies/semantics remain unverified and the processors were not executed.
- **Logical reconciliation:** SOURCE, RAW_EVIDENCE, PROCESSING_RUN, REJECT, REVIEW and CANDIDATE are `PARTIAL`; ASSESSMENT is `UNVERIFIED`; CANONICAL_HANDOFF is `MISSING`; `global_announcements` is `PUBLICATION-ONLY`. Existing physical tables are the extension base and are not replaced because naming or completeness differs.
- **Special-check outcome:** RAW evidence can be retained but immutability is not proven; rejection links to RAW but `ON DELETE SET NULL` can break permanent traceability; REVIEW exists as a constrained candidate state; candidates are physically separate and ticker-ineligible by default; run history exists without evidenced downstream run FKs; `source_registry` can remain the one registry; the 22-family taxonomy fits text/JSON foundations but lacks explicit version/tags/use-case/rail dimensions; canonical handoff is absent; processor RAW→publication behavior is unverified.
- **Verified boundary risks:** `global_announcements` defaults publication status to `review` and ticker eligibility to `true`; RAW links can be nulled; run lineage is not evidenced; SECURITY DEFINER processor bodies are unverified; and the repository-known legacy scraper direct-write path remains. RLS/policy metadata is stated factually without a security conclusion.
- **Gap outcome:** G0 records existing foundations; G1 covers mapping/documentation; G2 covers minimal lineage, preservation, taxonomy, handoff and ticker-default changes; G3 covers processor-definition/behavior; G4 covers the owner-controlled legacy-path retirement/cutover decision. No gap was implemented.
- **File created:** `docs/M33-G1-SUPABASE-SCHEMA-RECONCILIATION.md`.
- **Control records updated:** `docs/PROJECT_STATUS.md`; `docs/GPIR_BACKLOG.md`; `docs/MASTER_PROJECT_LOG.md`, including exact Step 5C commit evidence and achieved Step 5D milestone state.
...
- **Delivery / milestone outcome:** logical design milestone achieved on the work branch at `0bbdf5bdd0a0d1918308e241d9b451ab46450da7`; local and remote SHAs were verified equal. Step 5D physical reconciliation is recorded above as the completed follow-on.
...
- **Handoff:** branch push succeeded; GitHub PR creation through the available integration returned HTTP 403 (`Resource not accessible by integration`). Owner-controlled PR creation and review remain pending.
